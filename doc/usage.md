...
## Usage on Windows
1) Double click the `fno-stak.exe` file
2) Fill in the pool url settings, currency, username and password

`set FNOSTAK_NOWAIT=1` disable the dialog `Press any key to exit.` for non UAC execution.


## Usage on Linux & macOS
1) Open a terminal within the folder with the binary
2) Start the miner with `./fno-stak`

## Command Line Options

The miner allow to overwrite some of the settings via command line options.
Run `fno-stak --help` to show all available command line options.
...
```
docker run --rm -it -u $(id -u):$(id -g) --name fireice-uk/fno-stak -v "$PWD":/mnt fno-stak
docker stop fno-stak
docker run --rm -it -u $(id -u):$(id -g) --name fireice-uk/fno-stak -v "$PWD":/mnt fno-stak --config config.txt
```

Debug the docker image by getting inside:

```
docker run --entrypoint=/bin/bash --rm -it -u $(id -u):$(id -g) --name fireice-uk/fno-stak -v "$PWD":/mnt fno-stak
```
