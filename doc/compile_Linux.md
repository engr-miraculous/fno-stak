...
```
    ...
    sudo make install
    cd ..
    git clone http://github.com/fonero-project/fno-stak
    cd fno-stak
    mkdir build
    cd build
...
If you want to compile the binary without installing libraries / compiler or just compile binary for some other distribution, please check the [build_fno-stak_docker.sh script](scripts/build_fno-stak_docker/build_fno-stak_docker.sh).
...
```
    cmake -DCMAKE_LINK_STATIC=ON -DFNO-STAK_COMPILE=generic .
    make install
    cd bin\Release
