 ...
 */
#if defined(FNO_STAK_LARGEGRID) && (__CUDA_ARCH__ >= 300)
...
template<size_t ITERATIONS, uint32_t MEMORY, uint32_t MASK, fnostak_algo ALGO>
#ifdef FNO_STAK_THREADS
__launch_bounds__( FNO_STAK_THREADS * 4 )
#endif
...
template<size_t ITERATIONS, uint32_t MEMORY, fnostak_algo ALGO>
...
template<size_t ITERATIONS, uint32_t MASK, uint32_t MEMORY, fnostak_algo ALGO>
...
void cryptonight_core_cpu_hash(nvid_ctx* ctx, fnostak_algo miner_algo, uint32_t startNonce)
{
