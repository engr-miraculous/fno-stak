...
extern "C" void cryptonight_extra_cpu_prepare(nvid_ctx* ctx, uint32_t startNonce, fnostak_algo miner_algo)
{
...
extern "C" void cryptonight_extra_cpu_final(nvid_ctx* ctx, uint32_t startNonce, uint64_t target, uint32_t* rescount, uint32_t *resnonce,fnostak_algo miner_algo)
{
	int threadsperblock = 128;
	uint32_t wsize = ctx->device_blocks * ctx->device_threads;
...
#define FNOSTAK_PP_TOSTRING(str) FNOSTAK_PP_TOSTRING1(str)
	char const * archStringList = FNOSTAK_PP_TOSTRING(FNOSTAK_CUDA_ARCH_LIST);
#undef FNOSTAK_PP_TOSTRING
#undef FNOSTAK_PP_TOSTRING1
