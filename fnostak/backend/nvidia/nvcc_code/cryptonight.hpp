...
void cryptonight_extra_cpu_prepare(nvid_ctx* ctx, uint32_t startNonce, fnostak_algo miner_algo);
void cryptonight_extra_cpu_final(nvid_ctx* ctx, uint32_t startNonce, uint64_t target, uint32_t* rescount, uint32_t *resnonce,fnostak_algo miner_algo);
}
