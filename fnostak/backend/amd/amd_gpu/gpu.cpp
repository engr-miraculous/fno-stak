...
#include "fnostak/backend/cryptonight.hpp"
#include "fnostak/jconf.hpp"
#include "fnostak/picosha2/picosha2.hpp"
...
size_t InitOpenCLGpu(cl_context opencl_ctx, GpuContext* ctx, const char* source_code)
{
	...
	if(fnostak::params::inst().AMDCache == false || !clBinFile.good())
	{
		if(fnostak::params::inst().AMDCache)
...
size_t InitOpenCL(GpuContext* ctx, size_t num_gpus, size_t platform_idx)
{
	...
	source_code = std::regex_replace(source_code, std::regex("FNOSTAK_INCLUDE_WOLF_AES"), wolfAesCL);
	source_code = std::regex_replace(source_code, std::regex("FNOSTAK_INCLUDE_WOLF_SKEIN"), wolfSkeinCL);
	source_code = std::regex_replace(source_code, std::regex("FNOSTAK_INCLUDE_JH"), jhCL);
	source_code = std::regex_replace(source_code, std::regex("FNOSTAK_INCLUDE_BLAKE256"), blake256CL);
	source_code = std::regex_replace(source_code, std::regex("FNOSTAK_INCLUDE_GROESTL256"), groestl256CL);
...
size_t FNOSetJob(GpuContext* ctx, uint8_t* input, size_t input_len, uint64_t target, fnostak_algo miner_algo, uint32_t version)
{
	cl_int ret;
...
size_t FNORunJob(GpuContext* ctx, cl_uint* HashOutput, fnostak_algo miner_algo, uint32_t version)
{
	cl_int ret;
	cl_uint zero = 0;
