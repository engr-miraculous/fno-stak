...
#ifdef WIN32
		fn_starterBackend = (starterBackend_t) GetProcAddress(libBackend, "fnostak_start_backend");
		if (!fn_starterBackend)
		{
			std::cerr << "WARNING: backend plugin " << libName << " contains no entry 'fnostak_start_backend': " <<GetLastError()<< std::endl;
		}
#else
		// reset last error
		dlerror();
		fn_starterBackend = (starterBackend_t) dlsym(libBackend, "fnostak_start_backend");
		const char* dlsym_error = dlerror();
		if(dlsym_error)
		{
			std::cerr << "WARNING: backend plugin " << libName << " contains no entry 'fnostak_start_backend': " << dlsym_error << std::endl;
		}
#endif
	}
