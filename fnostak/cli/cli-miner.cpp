...
#include "fnostak/misc/executor.hpp"
...
#include "fnostak/misc/configEditor.hpp"
#include "fnostak/version.hpp"
#include "fnostak/misc/utility.hpp"

#ifndef CONF_NO_HTTPD
#	include "fnostak/http/httpd.hpp"
#endif
...
void help()
{
	...
	cout<<"  -o, --url URL              pool url and port, e.g. pool.fonero.org:3333"<<endl;
	cout<<"  -O, --tls-url URL          TLS pool url and port"<<endl;
	...
	cout<<"  FNOSTAK_NOWAIT             disable the dialog `Press any key to exit."<<std::endl;
...
std::string get_multipool_entry(bool& final)
{
	...
	std::string pool;
	std::cout<<"- Pool address: e.g. " << jconf::GetDefaultPool(fnostak::params::inst().currency.c_str()) << std::endl;
	std::cin >> pool;
...
int do_benchmark(int block_version)
{
	...
	std::vector<fnostak::iBackend*>* pvThreads;
	 ...
	 * \todo fix this issue
	 */
	fnostak::miner_work benchWork = fnostak::miner_work("", work, 84, 0, false, 0);
	printer::inst()->print_msg(L0, "Start a 60 second benchmark...");
	fnostak::globalStates::inst().switch_work(benchWork, dat);
	...
	for (uint32_t i = 0; i < pvThreads->size(); i++)
	{
		...
		auto bType = static_cast<fnostak::iBackend::BackendType>(pvThreads->at(i)->backendType);
