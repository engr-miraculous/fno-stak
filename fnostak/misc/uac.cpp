...
BOOL SelfElevate(const std::string& my_path, const std::string& params)
{
	...
	printer::inst()->print_msg(L0, "This window has been opened because fno-stak needed to run as administrator.  It can be safely closed now.");
...
VOID RequestElevation()
{
	...
	if(!fnostak::params::inst().allowUAC)
	{
	...
	SelfElevate(fnostak::params::inst().minerArg0, fnostak::params::inst().minerArgs);
}
