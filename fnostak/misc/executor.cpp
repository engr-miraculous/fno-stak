...
#include "fnostak/jconf.hpp"
#include "executor.hpp"
#include "fnostak/net/jpsock.hpp"

#include "telemetry.hpp"
#include "fnostak/backend/miner_work.hpp"
#include "fnostak/backend/globalStates.hpp"
#include "fnostak/backend/backendConnector.hpp"
#include "fnostak/backend/iBackend.hpp"

#include "fnostak/jconf.hpp"
#include "fnostak/misc/console.hpp"
#include "fnostak/donate-level.hpp"
#include "fnostak/version.hpp"
#include "fnostak/http/webdesign.hpp"
...
bool executor::get_live_pools(std::vector<jpsock*>& eval_pools, bool is_dev)
{
	...
	if(eval_pools.size() == 0)
	{
		if(!is_dev)
		{
			if(fnostak::globalStates::inst().pool_id != invalid_pool_id)
			{
				printer::inst()->print_msg(L0, "All pools are dead. Idling...");
				auto work = fnostak::miner_work();
				fnostak::pool_data dat;
				fnostak::globalStates::inst().switch_work(work, dat);
			}
...
void executor::eval_pool_choice()
{
	...
	if(goal->get_pool_id() != fnostak::globalStates::inst().pool_id)
	{
...
void executor::on_pool_have_job(size_t pool_id, pool_job& oPoolJob)
{
	...
	fnostak::miner_work oWork(oPoolJob.sJobID, oPoolJob.bWorkBlob, oPoolJob.iWorkLen, oPoolJob.iTarget, pool->is_nicehash(), pool_id);

	fnostak::pool_data dat;
	dat.iSavedNonce = oPoolJob.iSavedNonce;
	dat.pool_id = pool_id;

	fnostak::globalStates::inst().switch_work(oWork, dat);
...
void executor::on_miner_result(size_t pool_id, job_result& oResult)
{
	...
	const char* backend_name = fnostak::iBackend::getName(pvThreads->at(oResult.iThreadId)->backendType);
...
void executor::ex_main()
{
	...
	fnostak::miner_work oWork = fnostak::miner_work();

	// \todo collect all backend threads
	pvThreads = fnostak::BackendConnector::thread_starter(oWork);
	...
	telem = new fnostak::telemetry(pvThreads->size());
	...
	for(; i < pc; i++)
	{
		...
		if(!fnostak::params::inst().poolURL.empty() && fnostak::params::inst().poolURL == cfg.sPoolAddr)
		{
			auto& params = fnostak::params::inst();
	...
	if(!fnostak::params::inst().poolURL.empty() && !already_have_cli_pool)
	{
		auto& params = fnostak::params::inst();
		if(params.poolUsername.empty())
		{
			printer::inst()->print_msg(L1, "ERROR: You didn't specify the username / wallet address for %s", fnostak::params::inst().poolURL.c_str());
			win_exit();
		}
...
void executor::hashrate_report(std::string& out)
{
	...
	for( uint32_t b = 0; b < 4u; ++b)
	{
		...
		{
			size_t i;
			auto bType = static_cast<fnostak::iBackend::BackendType>(b);
			std::string name(fnostak::iBackend::getName(bType));
