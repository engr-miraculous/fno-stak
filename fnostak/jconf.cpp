...
struct fnostak_coin_algo
{
	const char* coin_name;
	fnostak_algo algo;
	fnostak_algo algo_root;
	uint8_t fork_version;
	const char* default_pool;
};

fnostak_coin_algo coin_algos[] = { 
	{ "fonero", cryptonight_monero, cryptonight, 3u, "pool.fonero.org:3333" }
};
...
{
	if(fnostak::params::inst().httpd_port == fnostak::params::httpd_port_unset)
		return prv->configValues[iHttpdPort]->GetUint();
	else
		return uint16_t(fnostak::params::inst().httpd_port);
}
...
{
	if(fnostak::params::inst().currency.length() > 0)
		return fnostak::params::inst().currency;
	else
...
{
	const char* default_example = "pool.fonero.org:3333";
